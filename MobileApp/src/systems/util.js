...
export const getFieldsBookingDetail = (request, masterData, t) => {
  ...
  const fields = [
    ...
    { label: t('booking.Ngày bắt đầu'), value: `${formatDate(request.startDate)} ${formatTime(request.startTime).slice(0, 5)}` },
    { label: t('booking.Ngày kết thúc'), value: `${formatDate(request.endDate || request.startDate)} ${formatTime(request.endTime).slice(0, 5)}` },
