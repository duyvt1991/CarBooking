        ...
        <div className="mb-4 flex items-center">
            ...
            <div className="w-full relative">
                {initForm[field].type === 'suggest' ? (<>
                    ...
                    (() => {
                        const optionsList = [];
                        if (initForm[field].optionsMasterDataKey && masterData[initForm[field].optionsMasterDataKey]) {
                            masterData[initForm[field].optionsMasterDataKey].forEach((type) => {
                                optionsList.push({
                                    value: initForm[field].isValueObject ? type : type.mkey,
                                    label: type.mvalue
                                });
                            });
                        }
                        if (initForm[field].options) {
                            initForm[field].options.forEach((type) => {
                                optionsList.push({
                                    value: initForm[field].isValueObject ? type : type.mkey,
                                    label: type.mvalue.includes(":") && type.mvalue.length === 5 ? type.mvalue : t(type.mvalue)
                                });
                            });
                        }

                        let placeholderText = `- ${t('common.Chọn')} -`;
                        if (request[`placeholder.${field}`] && !request[field]) {
                            let mappedPlaceholderValue = '';
                            if (initForm[field].optionsMasterDataKey && masterData[initForm[field].optionsMasterDataKey]) {
                                mappedPlaceholderValue = masterData[initForm[field].optionsMasterDataKey]?.find(option => option.mkey.toString() === request[`placeholder.${field}`].toString())?.mvalue;
                            }
                            if (!mappedPlaceholderValue && initForm[field].options) {
                                const foundOpt = initForm[field].options?.find(option => option.mkey.toString() === request[`placeholder.${field}`].toString());
                                if (foundOpt) {
                                    mappedPlaceholderValue = t(foundOpt.mvalue);
                                }
                            }
                            if (mappedPlaceholderValue) {
                                placeholderText = `- ${t('common.Chọn')} - (${mappedPlaceholderValue})`;
                            }
                        }
                        ...
                        return (
                            <CustomSelect
                                ...
                                className={`w-full ${errors[field] ? 'border-red-500 rounded-lg' : ''}`}
                                isValueObject={initForm[field].isValueObject}
                            />
                        );
                    })()
