...
function Home() {
  ...
  const isAuthorized = !masterData.userId || (masterData.userId !== "BitrixID-0" && masterData.userId !== "BitrixID-");
    ...
    <section className="p-1 space-y-6">
      ...
      {!isAuthorized && !loading && (
        <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-4 rounded shadow" role="alert">
          <p className="font-bold">{t('common.Lỗi xác thực')}</p>
          <p>
            {t('common.Không thể xác thực tài khoản Bitrix24 của bạn. Vui lòng kiểm tra phân quyền (Scope: Users) của ứng dụng hoặc thử tải lại trang.')}
          </p>
        </div>
      )}
