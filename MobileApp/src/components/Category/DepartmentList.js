...
function DepartmentList({
  ...
  const filterFields = [
    { name: 'id', placeholder: t('department.ID') },
    { name: 'mkey', placeholder: t('department.Mã phòng ban') },
    { name: 'mvalue', placeholder: t('department.Tên phòng ban') }
  ];

  const requestFields = [
    { name: 'id', label: t('department.ID'), render: (field, request) => request[field] },
    { name: 'mkey', label: t('department.Mã phòng ban'), render: (field, request) => request[field] },
    { name: 'mvalue', label: t('department.Tên phòng ban'), render: (field, request) => request[field] }
  ];
  
  const actionButtons = (request) => ([
    { label: t('common.Sửa'), className: 'bg-blue-500', action: (id) => handleEdit(id, routes.departmentForm.path) },
  ...
  return (
    <div className="p-1 relative">
      ...
      <div className="space-y-2 p-4 mb-4 bg-white shadow-md rounded-lg">
        <HeaderTableLayout 
          addNewPath={routes.departmentForm.path} 
          headerLabel={routes.departmentList.label} 
        />
        <FilterTableLayout 
...
export default withRequestData(DepartmentList, routes.departmentList.component);
