...
function EquipmentTypeList({
  ...
  const filterFields = [
    { name: 'id', placeholder: t('equipmentType.ID') },
    { name: 'mkey', placeholder: t('equipmentType.Mã loại thiết bị') },
    { name: 'mvalue', placeholder: t('equipmentType.Tên loại thiết bị') }
  ];

  const requestFields = [
    { name: 'id', label: t('equipmentType.ID'), render: (field, request) => request[field] },
    { name: 'mkey', label: t('equipmentType.Mã loại thiết bị'), render: (field, request) => request[field] },
    { name: 'mvalue', label: t('equipmentType.Tên loại thiết bị'), render: (field, request) => request[field] }
  ];
    
  const actionButtons = (request) => ([
    { label: t('common.Sửa'), className: 'bg-blue-500', action: (id) => handleEdit(id, routes.equipmentTypeForm.path) },
  ...
  return (
    <div className="p-1 relative">
      ...
      <div className="space-y-2 p-4 mb-4 bg-white shadow-md rounded-lg">
        <HeaderTableLayout 
          addNewPath={routes.equipmentTypeForm.path} 
          headerLabel={routes.equipmentTypeList.label} 
        />
        <FilterTableLayout 
...
export default withRequestData(EquipmentTypeList, routes.equipmentTypeList.component);
