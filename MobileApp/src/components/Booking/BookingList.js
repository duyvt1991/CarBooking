...
import { FaBan, FaCheck, FaRegHourglass } from 'react-icons/fa';
import { formatDateTime, formatDate, formatTime, formatBookingApproveStatus } from '../../systems/util';
...
function BookingList({
  ...
  const actionButtons = (request) => { 
    ...
    return ( 
        request?.isCancelled ? (
        [{ component: <span className="flex items-center justify-center gap-1"><FaBan className="text-red-500" /> {t('booking.Đã huỷ')}</span> }]
        ) : (
        isPastBooking ? (
          [{ component: <span className="flex items-center justify-center gap-1"><FaCheck className="text-green-500" /></span> }]
        ) : (
          !isBookingInProgress ? [
            { label: t('booking.Sửa'), className: 'bg-blue-500', action: (id) => handleEdit(id, routes.bookingForm.path) },
            { label: t('booking.Huỷ'), className: 'bg-red-500', action: (id) => handleEdit(id, routes.cancelBookingForm.path) }
          ] : [
            { component: <span className="inline-flex items-center justify-center gap-1"><FaRegHourglass className="text-yellow-500" /> {t('booking.Đang sử dụng')}</span> }
          ]
        )
      )
  )};
    ...
    <div className="p-1 relative">
      ...
      <div className="w-full overflow-x-auto">
        <div className="mb-0 ml-[1px] flex sticky left-0 space-x-2 text-sm">
          <button className={`p-2 rounded-t transition-colors duration-300 ${tempFilters.tab === "" ? "bg-green-600 text-white" : "bg-gray-200 hover:bg-gray-300"}`} onClick={() => navigate(routes.bookingList.path)}>{t('booking.Đặt phòng')}</button>
