...
class MasterData {
    ...
    public static function getMasterData() {
        global $USER;
        $logFile = dirname(dirname(__FILE__)) . '/page/debug_sync.log';
        @file_put_contents($logFile, "=== getMasterData called at " . date('Y-m-d H:i:s') . " ===\n", FILE_APPEND);
        @file_put_contents($logFile, "User ID: " . ($USER->GetID() ?: '0 (Not Logged In)') . "\n", FILE_APPEND);
        @file_put_contents($logFile, "Cookies: " . print_r($_COOKIE, true) . "\n", FILE_APPEND);
