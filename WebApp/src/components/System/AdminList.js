...
function AdminList({
  ...
  const filterFields = [
    ...
    { name: 'mkey', placeholder: t('admin.Esuhai User ID') },
    { name: 'mvalue', placeholder: t('admin.Esuhai User Name') }
  ];

  const requestFields = [
    { name: 'id', label: 'ID', render: (field, request) => request[field] },
    { name: 'mkey', label: t('admin.Esuhai User ID'), render: (field, request) => request[field] },
    { name: 'mvalue', label: t('admin.Esuhai User Name'), render: (field, request) => request[field] }
  ];

  const actionButtons = () => ([
    { label: t('common.Sửa'), className: 'bg-blue-500', action: (id) => handleEdit(id, routes.adminForm.path) },
  ...
  return (
    <div className="p-1 relative">
      ...
      <div className="space-y-2 p-4 mb-4 bg-white shadow-md rounded-lg">
        <HeaderTableLayout 
          addNewPath={routes.adminForm.path} 
          headerLabel={routes.adminList.label} 
        />
        <FilterTableLayout 
...
export default withRequestData(AdminList, routes.adminList.component);