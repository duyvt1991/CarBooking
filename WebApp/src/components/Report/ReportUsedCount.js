...
function ReportUsedCount() {
  ...
  const generateChartData = (data, formatter, label) => [
    ...
    ...Object.entries(data || {}).map(([key, value]) => [`${formatter(key)} (${value} ${t('report.lần')})`, value])
  ];
      ...
      <div className="p-0">
        <div className="grid grid-cols-1 my-4 sm:grid-cols-2 gap-4">
          <div className="p-4 border rounded-lg">
            ...
            />
            <p className="mt-2 text-gray-600 text-center">{t('report.Tổng')}: {totalUsagePurpose} {t('report.lần')}</p>
          </div>
          <div className="p-4 border rounded-lg">
            ...
            <Chart
              ...
              width="100%"
            />
            <p className="mt-2 text-gray-600 text-center">{t('report.Tổng')}: {totalLocale} {t('report.lần')}</p>
          </div>
        </div>
...
export default ReportUsedCount;
