...
function RoomList({
  ...
  const filterFields = [
    ...
    { name: 'mkey', placeholder: t('room.Mã phòng') },
    { name: 'mvalue', placeholder: t('room.Tên phòng') },
    { name: 'roomType', placeholder: t('room.Loại phòng'), type: 'select', options: masterData.roomTypes.map(type => ({ value: type.mkey, label: type.mvalue })) },
  ...
  const requestFields = [
    ...
    { name: 'roomType', label: t('room.Loại phòng'), render: (field, request) => formatRoomType(request[field], masterData) },
    { name: 'mkey', label: t('room.Mã phòng'), render: (field, request) => request[field] },
    { name: 'mvalue', label: t('room.Tên phòng'), render: (field, request) => request[field] },
