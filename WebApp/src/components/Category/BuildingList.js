...
function BuildingList({
  ...
  const filterFields = [
    { name: 'id', placeholder: t('building.ID') },
    { name: 'mkey', placeholder: t('building.Mã toà nhà') },
    { name: 'mvalue', placeholder: t('building.Tên toà nhà') }
  ];

  const requestFields = [
    { name: 'id', label: t('building.ID'), render: (field, request) => request[field] },
    { name: 'mkey', label: t('building.Mã toà nhà'), render: (field, request) => request[field] },
    { name: 'mvalue', label: t('building.Tên toà nhà'), render: (field, request) => request[field] },
    { name: 'address', label: t('building.Địa chỉ'), render: (field, request) => request[field] }
  ];

  const actionButtons = (request) => ([
    { label: t('common.Sửa'), className: 'bg-blue-500', action: (id) => handleEdit(id, routes.buildingForm.path) },
  ...
  return (
    <div className="p-1 relative">
      ...
      <div className="space-y-2 p-4 mb-4 bg-white shadow-md rounded-lg">
        <HeaderTableLayout 
          addNewPath={routes.buildingForm.path} 
          headerLabel={routes.buildingList.label} 
        />
        <FilterTableLayout 
...
export default withRequestData(BuildingList, routes.buildingList.component);
